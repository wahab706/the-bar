...
import {
    Page, Layout, Card, Scrollable, Text, Stack, ButtonGroup, Button, PageActions, Form, FormLayout, TextField, Icon,
    Listbox, EmptySearchResult, AutoSelection, Toast, Loading, List, TextContainer, Banner, Modal, Avatar, Tabs
} from '@shopify/polaris';
import {
    SearchMinor, ChevronDownMinor, ChevronUpMinor,
} from '@shopify/polaris-icons';
import {
    SkeltonPageForProductDetail, getAccessToken, InputField, CheckBox, CustomBadge
} from '../../components'
...
export function DiscountDetail() {
    ...
    const editDiscount = async (id) => {
        setLoading(true)
        try {
            const response = await axios.get(`${apiUrl}/api/automatic_discount/${id}/edit`, {
                headers: { "Authorization": `Bearer ${getAccessToken()}` }
            })
            ...
            if (response.data.errors) {
                ...
                setDiscount({
                    ...
                    products: discountResponse?.products && discountResponse?.products.split(','),
                    variants: discountResponse?.variants && discountResponse?.variants.split(','),
                })
                ...
                if (discountResponse?.endDate == null) {
                    setActiveDates({
                        isEndDate: false,
                        deleteAfterEndDate: convertNumberToBoolean(discountResponse?.deleteAfterEndDate),
                ...
                if (response.data.data?.collections != null) {
                    ...
                    let collectionsArray = CollectionsArraySet(collections, 'set', 'collection')
                    let ids = new Set(globalCollections.map(d => d.id));
    ...
    const deleteDiscount = async () => {
        ...
        try {
            const response = await axios.delete(`${apiUrl}/api/automatic_discount/${discountId}/delete`, {
                headers: { "Authorization": `Bearer ${getAccessToken()}` }
            })
    ...
    const updateDiscount = async () => {
        let startDate = dateFormat(`${activeDates.startDate} ${activeDates.startTime}`, "UTC:yyyy-mm-dd'T'HH:MM:ss'Z'");
        let endDate = dateFormat(`${activeDates.endDate} ${activeDates.endTime}`, "UTC:yyyy-mm-dd'T'HH:MM:ss'Z'");
        ...
        try {
            const response = await axios.put(`${apiUrl}/api/automatic_discount/${discountId}/update`, data, {
                headers: { "Authorization": `Bearer ${getAccessToken()}` }
            })
    ...
    const handleUpdateDiscountSubmit = (e) => {
        ...
        if (discount.appliesTo == 'all') {
            ...
            if (discount.products == null || discount.products == 'null' || discount.products == [] || discount.products?.length < 1) {
            ...
            if (discount.collections == null || discount.collections == 'null' || discount.collections == [] || discount.collections?.length < 1) {
        ...
        <div className='Discount-Detail-Page'>
            ...
            {loading ?
                ...
                <Page
                    ...
                    title={discount.code}
                    titleMetadata={discount.status && <CustomBadge value={discount.status} type='discount' />}
                    primaryAction={
                        {
