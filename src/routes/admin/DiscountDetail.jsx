...
export function DiscountDetail() {
      ...
      arr.push({
        ...
        label: (
          <>
            ...
            <span>
              {`${getSelectedVariantsLength(item.id)}/${
                item.totalVariants
              } selected`}
            </span>
          </>
      ...
      if (variants?.edges?.length > 0) {
        variants?.edges?.map((item) => {
          arr.push({
            ...
            productId: item.node.product.id.replace(
              "gid://shopify/Product/",
              ""
            ),
          });
  ...
  const getProducts = async () => {
    ...
    try {
      const response = await axios.get(
        `${apiUrl}/api/shopify/products?title=${queryValue}&next=${nextPageCursor}&status=`,
        {
      ...
      if (response.data.errors) {
        ...
        if (products.edges?.length > 0) {
          ...
          let global = [
            ...globalProducts,
            ...productsArray.filter((d) => !ids.has(d.id)),
          ];
          setProductsList(list);
          setAllProducts(all);
  ...
  function CollectionsArraySet(collections, type, value) {
    ...
    if (type == "get") {
      ...
      collections?.nodes?.map((item) => {
        collectionsArray.push({
          id: item.id.replace("gid://shopify/Collection/", ""),
          title: item.title,
          productsCount: item.productsCount,
          image: item.image?.transformedSrc,
        });
      });
      if (value == "collection") {
        return collectionsArray;
  ...
  const getCollections = async () => {
    ...
    try {
      const response = await axios.get(
        `${apiUrl}/api/shopify/collections?title=${collectionQueryValue}&next=${nextPageCursorCollection}`,
        {
          headers: { Authorization: `Bearer ${getAccessToken()}` },
        }
      );
      ...
      if (response.data.errors) {
        ...
        if (collections.edges?.length > 0) {
          let collectionsArray = CollectionsArraySet(
            collections,
            "get",
            "collection"
          );
          ...
          let global = [
            ...globalCollections,
            ...collectionsArray.filter((d) => !ids.has(d.id)),
          ];
          setCollectionsList(list);
          setAllCollections(all);
          ...
          setNextPageCursorCollection(
            CollectionsArraySet(collections, "get", "nextPage")
          );
  ...
  const LazyLoadingMarkupCollection = collectionsLoading ? (
    <Listbox.Loading
      accessibilityLabel={`${
        collectionQueryValue ? "Filtering" : "Loading"
      } Products`}
    />
  ) : allCollections?.length > 0 && hasNextPageCollection ? (
    <Button onClick={handleCollectionsPagination}>Load more...</Button>
  ) : null;

  const noResultsMarkupCollection =
    !collectionsLoading && allCollections.length == 0 ? (
      <EmptySearchResult title="No Collection found" />
    ) : null;
  ...
  const handleDiscountCheckbox = (e) => {
    ...
    if (e.target.name == "isEndDate") {
      ...
      setActiveDates({
        ...activeDates,
        deleteAfterEndDate: !activeDates.deleteAfterEndDate,
      });
    }
  };
  ...
  const editDiscount = async (id) => {
    ...
    try {
      const response = await axios.get(
        `${apiUrl}/api/automatic_discount/${id}/edit`,
        {
          headers: { Authorization: `Bearer ${getAccessToken()}` },
        }
      );

      // console.log('editDiscount response: ', response.data);
      if (response.data.errors) {
        setToastMsg(response.data.message);
        setErrorToast(true);
      } else {
        let discountResponse = response.data.data;
        setDiscountId(discountResponse?.id);
        setDiscount({
          ...discount,
          title: discountResponse?.title,
          code: discountResponse?.code,
          type: discountResponse?.type,
          value: discountResponse?.value,
          minimumRequirement: discountResponse?.minimumRequirement,
          minimumValue: discountResponse?.minimumValue,
          status: discountResponse?.status,
          appliesTo: discountResponse?.appliesTo,
          collections: discountResponse?.collections,
          products:
            discountResponse?.products && discountResponse?.products.split(","),
          variants:
            discountResponse?.variants && discountResponse?.variants.split(","),
        });

        if (discountResponse?.endDate == null) {
          setActiveDates({
            isEndDate: false,
            deleteAfterEndDate: convertNumberToBoolean(
              discountResponse?.deleteAfterEndDate
            ),
            startDate: dateFormat(discountResponse?.startDate, "yyyy-mm-dd"),
            startTime: dateFormat(discountResponse?.startDate, "HH:MM"),
            endDate: dateFormat(discountResponse?.startDate, "yyyy-mm-dd"),
            endTime: dateFormat(discountResponse?.startDate, "HH:MM"),
          });
        } else {
          setActiveDates({
            isEndDate: true,
            ...
            startDate: dateFormat(discountResponse?.startDate, "yyyy-mm-dd"),
            startTime: dateFormat(discountResponse?.startDate, "HH:MM"),
            endDate: dateFormat(discountResponse?.endDate, "yyyy-mm-dd"),
            endTime: dateFormat(discountResponse?.endDate, "HH:MM"),
          });
        }

        //get selected variants of discount
        if (response.data.data?.variants != null) {
          let products =
            response.data.data?.proudct_data?.original?.data?.body?.products;
        ...
        let checked = [];
        if (response.data.data?.variants != null) {
          checked = response.data.data?.variants?.split(",");
        }
        setPreviousCheckedVariants(checked);
        setCheckedVariants(checked);

        //get selected collections of discount
        if (response.data.data?.collections != null) {
          let collections = response.data.data?.collection_data?.body?.data;
          let collectionsArray = CollectionsArraySet(
            collections,
            "set",
            "collection"
        ...
        if (response.data.data?.collections != null) {
          checkedCollection = response.data.data?.collections?.split(",");
        }
        setCheckedVariantsCollections(checkedCollection);
        setPreviousCheckedVariantsCollections(checkedCollection);
      ...
      console.warn("editDiscount Api Error", error.response);
  ...
  useEffect(() => {
    if (toggleLoadData) {
      editDiscount(params.discountId);
    }
  }, [toggleLoadData]);
  ...
  const deleteDiscount = async () => {
    ...
    try {
      const response = await axios.delete(
        `${apiUrl}/api/automatic_discount/${discountId}/delete`,
        {
          headers: { Authorization: `Bearer ${getAccessToken()}` },
        }
      );
  ...
  const updateDiscount = async () => {
    ...
    let data = {
      ...
      products: products,
      variants: variants,
      deleteAfterEndDate: convertBooleanToNumber(
        activeDates.deleteAfterEndDate
      ),
      startDate: startDate,
      endDate: endDate,
    };
    ...
    try {
      const response = await axios.put(
        `${apiUrl}/api/automatic_discount/${discountId}/update`,
        data,
        {
          headers: { Authorization: `Bearer ${getAccessToken()}` },
        }
      ...
      // console.log('updateDiscount response: ', response.data);
      if (response.data.errors) {
        setToastMsg(response.data.message);
        setErrorToast(true);
      } else {
        setToastMsg("Discount Updated Sucessfully");
        collectionsModalClose();
        productsModalClose();
        setSucessToast(true);
      ...
      console.warn("updateDiscount Api Error", error.response);
      if (error.response?.data?.message) {
        setToastMsg(error.response?.data?.message);
      } else {
        setToastMsg("Server Error");
  ...
  const handleUpdateDiscountSubmit = (e) => {
    e.preventDefault();
    if (discount.appliesTo == "all") {
      ...
      if (
        discount.collections == null ||
        discount.collections == "null" ||
        discount.collections == [] ||
        discount.collections?.length < 1
      ) {
    ...
    <div className="Discount-Detail-Page">
      ...
      >
        <Modal.Section>
          <div className="Countries-Modal-Section Products-Modal-Section">
            ...
            <Tabs
              tabs={productModalTabs}
              selected={productTab}
              onSelect={handleProductTabChange}
            >
              {(() => {
                switch (productTab) {
                  case 0:
                    return (
                      <>
                        <CheckboxTree
                          ...
                          onCheck={(checked) =>
                            SetCustomVariantsSelected(checked, "all")
                          }
                    ...
                    return (
                      <span className="Selected-Tab-Modal">
                        <CheckboxTree
                          ...
                          expanded={expandedProduct}
                          onCheck={(checked) =>
                            SetCustomVariantsSelected(checked, "selected")
                          }
                          onExpand={(expanded) => setExpandedProduct(expanded)}
                          icons={{
                            check: <img src={FillCheckBox} alt="checkbox" />,
      ...
      >
        <Modal.Section>
          <div className="Countries-Modal-Section Products-Modal-Section Collections-Modal-Section">
            ...
            <Tabs
              tabs={collectionModalTabs}
              selected={collectionTab}
              onSelect={handleCollectionTabChange}
            >
              {(() => {
                    ...
                    return (
                      <>
                        <CheckboxTree
                          ...
                          onCheck={(checked) =>
                            SetCustomVariantsSelectedCollection(checked, "all")
                          }
                          onExpand={(expanded) =>
                            setExpandedCollection(expanded)
                          }
                          icons={{
                            check: <img src={FillCheckBox} alt="checkbox" />,
                            halfCheck: (
                    ...
                    return (
                      <span className="Selected-Tab-Modal">
                        <CheckboxTree
                          ...
                          expanded={expandedCollection}
                          onCheck={(checked) =>
                            SetCustomVariantsSelectedCollection(
                              checked,
                              "selected"
                            )
                          }
      ...
      {loading ? (
        ...
        <Page
          ...
          titleMetadata={
            discount.status && (
              <CustomBadge value={discount.status} type="discount" />
            )
          }
          primaryAction={{
        ...
        >
          ...
          <Layout>
            <Layout.Section>
              <Form onSubmit={handleUpdateDiscountSubmit}>
                <FormLayout>
                  ...
                  <Card sectioned title="Automatic discount">
                    <Text variant="bodyMd" as="p" fontWeight="regular">
                      ...
                      <a
                        href="https://help.checkify.pro/en/articles/5463278-autodiscounts"
                        target="_blank"
                  ...
                  <Card sectioned title="Minimum purchase requirements">
                    <div className="Requirements-Section">
                      ...
                      <CheckBox
                        ...
                        value="amount"
                        checked={
                          discount.minimumRequirement == "amount" && true
                        }
                        onChange={handleDiscount}
                      ...
                      <CheckBox
                        ...
                        value="quantity"
                        checked={
                          discount.minimumRequirement == "quantity" && true
                        }
                        onChange={handleDiscount}
                            ...
                            <Card sectioned subdued>
                              <Stack>
                                <Text
                                  ...
                                  fontWeight="medium"
                                >
                                  {discount.variants?.length} Variants (of{" "}
                                  {discount.products?.length} Product) selected
                                </Text>
                                <Button
                                  primary
                                  onClick={handleSelectProductsModal}
            ...
            <Layout.Section oneThird>
                  ...
                  <Card.Section>
                    ...
                    <List type="bullet">
                      {discount.value ? (
                        discount.type == "fixed" ? (
                          <List.Item>
                            {user?.currency_symbol} {discount.value} off
                          </List.Item>
                        ) : (
                          <List.Item>
                            {discount.value}% off all products
                          </List.Item>
                        )
                      ) : (
                        ""
                      ...
                      {discount.minimumRequirement == "amount" &&
                      discount.minimumValue ? (
                        <List.Item>
                          Minimum purchase of ${discount.minimumValue}
                        </List.Item>
                      ) : (
                        ""
                      )}
                      ...
                      discount.minimumValue ? (
                        <List.Item>
                          Minimum quantity of {discount.minimumValue}
                        </List.Item>
                      ) : (
                        ""
                      )}
                      ...
                      <List.Item>
                        Active from{" "}
                        {dateFormat(activeDates.startDate, "dd/mm/yyyy")}
                      </List.Item>

                      {activeDates.isEndDate && (
                        <List.Item>
                          End at {dateFormat(activeDates.endDate, "dd/mm/yyyy")}
                        </List.Item>
                      )}
                    </List>
                  </Card.Section>
              ...
              <Card sectioned subdued title="Important">
                <List>
                  ...
                  <List.Item>
                    Customers will be able to override the automatic discount
                    with their discount code.
                  </List.Item>
                  <List.Item>
                    If two automatic discounts are applied to the order, the
                    most favorable for the buyer will be displayed.
                  </List.Item>

                  <br />
                  <a
                    href="https://help.checkify.pro/en/articles/5463278-autodiscount"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
